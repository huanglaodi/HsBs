...
public class FKDataHS105_GLog {

    public final int STRUCT_SIZE = 20;        // sizeof(GLOG_DATA_CIF12)
    ...
    public int tmLog;
    public FKDataHS105_GLog(byte[] abytLog){

        if (abytLog.length != STRUCT_SIZE)
            return;
        UserId = MyUtil.byte2int(abytLog, 0);
        ...
        IoMode = MyUtil.byte2int(Arrays.copyOfRange(abytLog, 12, 16));

        VerifyMode = MyUtil.byte2int(Arrays.copyOfRange(abytLog, 16, 20));
    }
